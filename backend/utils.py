...
def make_conversion(message):
    ...
    try:
        ...
        audio_file = os.path.join(abspath(dirname(__file__)), current_app.config.get('AUDIO_STORAGE'),
...
def calc_order(products_order, order, products):
    try:
        ...
        message = get_messages('CLOSING_ORDER_MESSAGE').format(order.id, money_ext, order_itens_text, order.quantity)
...
def money_format(price):
    try:
        ...
        money_brl_ext = format_currency(price, current_app.config.get('COIN'), locale=dates.LC_TIME)
...
def get_messages(key):
    ...
    with codecs.open(file_path, mode='r', encoding=current_app.config.get('TEXT_ENCODE')) as msg_file:
