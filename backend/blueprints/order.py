...
from schemas import OrderSchema, ProductsOrderSchema
from status_code_enum import StatusCodeEnum
from utils import calc_order, call_order, sent_order, get_messages, money_format
...
def close():
    ...
    try:
        ...
        products_order = ProductsOrderSchema().dump(json_data).data
        ...
        if not products:
            return json_response(status_=StatusCodeEnum.HTTP_404_NOT_FOUND, result=get_messages('PRODUCT_NOT_FOUND'))
        if order.dt_closing:
            return json_response(status_=StatusCodeEnum.HTTP_400_BAD_REQUEST,
        ...
        return json_response(status_=StatusCodeEnum.HTTP_201_CREATED,
                             result=get_messages('CLOSING_ORDER_MESSAGE_POPUP').format(order.id, money_ext_brl,
                                                                                       order.quantity))
...
def sent(order_id):
    ...
    if not order_id:
        return json_response(status_=StatusCodeEnum.HTTP_400_BAD_REQUEST,
                             result=get_messages('ORDER_NOT_FOUND'))
    order = Order.query.filter(Order.id == order_id and not Order.is_sentent and Order.is_called).one()
    if order:
        sent_order(order)
        return json_response(status_=StatusCodeEnum.HTTP_200_OK,
...
def call(order_id):
    ...
    order = Order.query.filter(Order.id == order_id and not Order.is_sentent and not Order.is_called).one()
    if order:
        call_order(order)
        return json_response(status_=StatusCodeEnum.HTTP_200_OK,
