...
def add():
    ...
    try:
        ...
        db_session.commit()
        return json_response(status_=StatusCodeEnum.HTTP_201_CREATED, result=get_messages('PRODUCT_CREATED'))
    except:
        db_session.rollback()
        raise
    finally:
...
def edit(product_id):
    ...
    try:
        ...
        db_session.add(product)
        db_session.commit()
        return json_response(status_=StatusCodeEnum.HTTP_202_ACCEPTED, result=get_messages('PRODUCT_CHANGED'))
    except:
        db_session.rollback()
        raise
    finally:
...
def delete(product_id):
    ...
    try:
        ...
        db_session.commit()
        return json_response(status_=StatusCodeEnum.HTTP_202_ACCEPTED, result=get_messages('PRODUCT_REMOVED'))
    except:
        db_session.rollback()
        raise
    finally:
...
@products_blueprint.route('product/<product_id>', endpoint='products_by_id', defaults={'product_name': None},
                          methods=['GET'])
@products_blueprint.route('product/<product_name>/name', endpoint='products_by_name', defaults={'product_id': None},
                          methods=['GET'])
@products_blueprint.route('product/', endpoint='all_products', defaults={'product_id': None, 'product_name': None},
                          methods=['GET'])
...
def get(product_id, product_name):
    ...
    try:
        if product_id:
            ...
            result = ProductSchema(many=True).dump(Product.query.filter(Product.name.like('{}%'.format(product_name))))
        else:
